...
from wtforms import StringField, PasswordField, SubmitField, IntegerField, SelectField, HiddenField, BooleanField
from wtforms.validators import DataRequired, NumberRange

class LoginForm(FlaskForm):
    username    = StringField('Uživatelské jméno', validators=[DataRequired()])
    password    = PasswordField('Heslo', validators=[DataRequired()])
    remember_me = BooleanField('Pamatovat si mě')
...
class InventuraForm(FlaskForm):
    submit_save    = SubmitField('Uložit inventuru')
    submit_preview = SubmitField('Náhled')
    submit_confirm = SubmitField('Potvrdit inventuru')
